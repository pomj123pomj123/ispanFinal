<template><div  style="max-width: 100%;
  ...
  <div class="container">
    ...
    <p v-if="emailExists !== null">
      ...
      <i :class="['fas', emailExists ? 'fa-check-circle' : 'fa-times-circle']" :style="{ color: emailExists ? 'green' : 'red' }"></i>
    </p>
    ...
    checkEmail() {
      axios.get(`/api/customers/email/${this.email}`)
        .then(response => {
          ...
          this.sendVerificationCode(); // Send verification code if email exists
          this.username = response.data.username;
        })
        .catch(error => {
          if (error.response.status === 404) {
            // Show SweetAlert for email not found
            Swal.fire({
              icon: 'error',
              title: '找不到email',
              text: '此email未註冊'
            });
            this.emailExists = false; // Email does not exist
          } else {
            console.error('Error checking email:', error);
          }
        });
    },
    sendVerificationCode() {document.getElementById('loadingIndicator').style.display = 'block';
