      ...
      <table class="table table-bordered table-hover table-striped member-list">
        ...
        <tbody>
          <tr v-for="member in members" :key="member.mrId">
            ...
            <td>
              <img :src="`http://192.168.31.160:8080/member/pages/${member.mrId}`" alt="會員卡" class="photo-img" style="max-width: 100px; max-height: 100px;" />
            </td>
...
export default {
  ...
  methods: {
    async showFormDialog() {console.log("999");
    ...
    async editMember(member) {console.log("999");
      const { value: formValues } = await Swal.fire({
        title: '編輯會員等級',
        html: `
